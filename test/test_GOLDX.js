...
describe("GOLDX Token", () => {
  beforeEach(async () => {
    [team, marketing, treasury, rewardVault, multiSigVault, ...users] =
  ...
  describe("Transactions & Fees distribution", () => {
    it("Should take fees for transaction and distribute it to holders, treasury, burn wallet and referrals", async () => {
      ...
      // result = 10 - 1 + 0.7
      let expectedHolderAmount = amountWithFees.add(
        feeAmount.mul(TOHOLDERS).div(100)
      );
      expect(balancesAfter.user1).to.be.equal(
        balancesBefore.user1.add(expectedHolderAmount)
      );
      expect(balancesAfter.treasury).to.be.equal(
        feeAmount.mul(TOTREASURY).div(100)
      );
      ...
      let sum = Object.values(balancesAfter)
        .map((el) => el * 1, 0)
        .reduce((a, b) => a + b, 0);
    ...
    it("If user is in the blacklist, transaction is not executed", async () => {
      ...
      await expect(goldX.transfer(users[0].address, amount)).to.be.revertedWith(
        "GOLDX: USER IS BLACKLISTED"
      );
    });
    ...
    it("New fee amount must be in 0-15% range", async () => {
      let newFeeAmount = 16;
      await expect(goldX.setFees(newFeeAmount)).to.be.revertedWith(
        "GOLDX: 0% >= TRANSACTION FEE <= 15%"
      );
    });
    it("Owner can change fee distributon", async () => {
      ...
      let toBurn = 15;
      let toReferrals = 15;
      await goldX.setFeeDistribution(
        toHolders,
        toTreasury,
        toBurn,
        toReferrals
    ...
    it("Only owner can set fees", async () => {
      let newFeeAmount = 16;
      await expect(
        goldX.connect(users[0]).setFees(newFeeAmount)
    ...
    it("Only owner can set new fee distributions", async () => {
      ...
      await expect(
        goldX
          .connect(users[0])
          .setFeeDistribution(toHolders, toTreasury, toBurn, toReferrals)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
  describe("Superadmins", () => {
    ...
    it("Owner and superadmins can add/remove users to the whitelist", async () => {
      ...
      await expect(
        goldX.connect(users[1]).addToWhitelist(users[2].address)
      ).to.be.revertedWith(
        `AccessControl: account ${users[1].address.toLowerCase()} is missing role ${SUPERADMIN}`
      );
      await expect(
        goldX.connect(users[1]).removeFromWhitelist(users[2].address)
      ).to.be.revertedWith(
        `AccessControl: account ${users[1].address.toLowerCase()} is missing role ${SUPERADMIN}`
      );
    });
    it("Owner and superadmins can add/remove users to the blacklist", async () => {
      ...
      await expect(
        goldX.connect(users[1]).addToBlacklist(users[2].address)
      ).to.be.revertedWith(
        `AccessControl: account ${users[1].address.toLowerCase()} is missing role ${SUPERADMIN}`
      );
      await expect(
        goldX.connect(users[1]).removeFromBlacklist(users[2].address)
      ).to.be.revertedWith(
        `AccessControl: account ${users[1].address.toLowerCase()} is missing role ${SUPERADMIN}`
      );
    });
    it("Owner and superadmins can put the token on pause", async () => {
      ...
      await expect(goldX.connect(users[1]).unpause()).to.be.revertedWith(
        `AccessControl: account ${users[1].address.toLowerCase()} is missing role ${SUPERADMIN}`
      );
      ...
      await expect(goldX.connect(users[1]).pause()).to.be.revertedWith(
        `AccessControl: account ${users[1].address.toLowerCase()} is missing role ${SUPERADMIN}`
      );
    });
    it("Transfers and other functions during pause are prohibited", async () => {
      await goldX.pause();
      await expect(goldX.addToWhitelist(users[0].address)).to.be.revertedWith(
        "Pausable: paused"
      );
      await expect(
        goldX.transfer(users[0].address, 100000000)
  ...
  describe("Referral program", () => {
    ...
    it("User should wait for the cooldown to change his referrer again", async () => {
      ...
      await expect(
        goldX.connect(users[1]).setReferrer(users[2].address)
      ...
      for (let i = 0; i < 5; i++) {
        for (let j = 0; j < 5; j++) {
          if (i != j)
            await goldX
              .connect(users[i])
              .transfer(users[j].address, parseEther("100"));
        }
      }
      let balancesAfter = await getBalances();
      let totalSupplyAfter = await goldX.totalSupply();
