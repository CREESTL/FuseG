...
describe("Multi-signature vault", () => {
  beforeEach(async () => {
    [fuseG, team, marketing, treasury, alice, bob, charlie, eva] =
      await ethers.getSigners();

    let MultiSigVault = await ethers.getContractFactory("MultiSigVault");
    multiSigVault = await MultiSigVault.deploy([
      alice.address,
      bob.address,
      charlie.address,
    ]);
  ...
  describe("Base", () => {
    it("Successfully initializes the vault", async () => {
      expect(await multiSigVault.getRewardVault()).to.be.equal(
        rewardVault.address
      );
  ...
  describe("Proposals", () => {
    it("Signer can submit a proposal", async () => {
      ...
      await expect(
        multiSigVault
          .connect(alice)
          .submitProposal(0, marketing.address, amount)
      )
    ...
    it("Only signer can submit a proposal", async () => {
      let amount = parseEther("10000");
      await expect(
        multiSigVault.submitProposal(0, marketing.address, amount)
    ...
    it("Only signer can confirm a proposal", async () => {
      ...
      await multiSigVault
        .connect(alice)
        .submitProposal(0, marketing.address, amount);
      await expect(multiSigVault.confirmProposal(0)).to.be.revertedWith(
        "MV: NOT A SIGNER"
      );
    });
    it("Only signer can execute a proposal", async () => {
      ...
      await multiSigVault
        ...
        .submitProposal(0, marketing.address, amount);
      await multiSigVault.connect(alice).confirmProposal(0);
      await expect(multiSigVault.executeProposal(0)).to.be.revertedWith(
        "MV: NOT A SIGNER"
      );
    });
    it("Signer can't confirm same proposal twice", async () => {
      ...
      await multiSigVault.connect(alice).confirmProposal(0);
      await expect(
        multiSigVault.connect(alice).confirmProposal(0)
    ...
    it("Signer can't confirm proposal if it doesn't exist", async () => {
      ...
      await multiSigVault
        ...
        .submitProposal(0, marketing.address, amount);
      await expect(
        multiSigVault.connect(alice).confirmProposal(1)
    ...
    it("Signer can't execute proposal if it's already executed", async () => {
      ...
      await expect(
        multiSigVault.connect(charlie).executeProposal(0)
      ).to.be.revertedWith("MV: PROPOSAL ALREADY EXECUTED");
    });
    it("Signer can't revoke confirmation if he hasn't confirmed a proposal", async () => {
      ...
      await multiSigVault
        ...
        .submitProposal(0, marketing.address, amount);
      await expect(
        multiSigVault.connect(alice).revokeConfirmation(0)
