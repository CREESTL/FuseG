...
async function main() {
  ...
  if (network.name === "polygon") {
    ...
    url = "https://mumbai.polygonscan.com/address/" + multisig.address + "#code";
  } else if (network.name === "ethereum") {
  ...
  console.log(`[${contractName}]: Start of Deployment...`);
  _contractProto = await ethers.getContractFactory(contractName);
  contractDeployTx = await _contractProto.deploy(accounts.TEAM, accounts.MARKETING, accounts.TREASURY, reward.address, multisig.address )
